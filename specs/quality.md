...
it("should fetch movies from TMDB", async () => {
  const mockMovies = [{ id: 1, title: "Cinema Paradiso" }];
  (tmdbFetch as jest.Mock).mockResolvedValue({ results: mockMovies });

  const { result } = renderHook(() => useTMDBSearch("cinema"));

  await act(async () => {});

  expect(tmdbFetch).toHaveBeenCalledWith("/search/movie", { query: "cinema" });
...
- Nunca expor `TMDB_API_KEY` no client-side
- Usar rota proxy (`/api/tmdb/search`) se necessário no client
- Rate limit: 40 requests/10s — implementar retry com exponential backoff
- Imagens: usar `https://image.tmdb.org/t/p/w500{poster_path}`
